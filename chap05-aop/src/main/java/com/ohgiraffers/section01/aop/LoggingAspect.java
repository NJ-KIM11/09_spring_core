...
public class LoggingAspect {
    ...
    /*
    *  @Aspect
    *   - pointcut 과 advice 를 하나의 클래스 단위로 정의하기 위한 annotation이다.
    *
    ...
    * */
    ...
    @Pointcut("execution(* com.ohgiraffers.section01.aop.*Service.*(..))")  // 조인 포인트만 잡아주고 메소드는 실행되지 않는다..
    ...
    public void logBefore(JoinPoint joinPoint) {
        ...
        if (joinPoint.getArgs().length > 0) {
            System.out.println("before jointpoint.getArgs()[0] : " + joinPoint.getArgs()[0]);
        }
    }
    ...
    @AfterReturning(pointcut = "logPointCut()", returning = "result")       // result 는 메소드의 반환값
